 ...
 */
function onOpen(e) {
  const adminMenu = SpreadsheetApp.getUi().createMenu('Admin');
  ...
  adminMenu.addSubMenu(settingsMenu);
  adminMenu.addSubMenu(squareMenu);

  if (isSupportMenuEnabled_()) {
    const supportMenu = SpreadsheetApp.getUi()
      .createMenu('Support tools')
      .addItem('Preview Square payment sync', 'previewSquareCommercialSyncFromMenu');
    adminMenu.addSubMenu(supportMenu);
  }
